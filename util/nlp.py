...
from util.output_vectors import (positive_answers, negative_answers,
                                neutral_answers, propositions, not_sure)
...
def get_word_polarity(word_input):
    ...
    data = {}
    with open('files/SentiLex-lem-PT02.txt', 'r') as f:
...
def get_offense_level(text):
    ...
    """
    count = 0
    with open('files/hateset.txt', 'r') as f:
...
def get_random_blablabla():
    """
    Retorna uma mensagem aleatória só pra encher linguiça.
    """
    filled_sausage = ''.join(choice(i) for i in propositions)
    conclusion = ''.join(choice(i) for i in not_sure)
    response = '{} {} {} Oh céus, o que estou dizendo...'.format(
        choice(opinions[0]),
        filled_sausage,
        conclusion
    )
